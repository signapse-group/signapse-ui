...
- [ ] Required regression evidence passes; an absent seam is reported as an unresolved evidence gap, not automatically waived
- [ ] All `[DEBUG-...]` instrumentation removed (`grep` the prefix)
- [ ] Throwaway prototypes deleted (or moved to a clearly-marked debug location)
- [ ] The hypothesis that turned out correct is stated in the commit / PR message — so the next debugger learns

Report architectural improvements outside the fix scope as optional follow-up with their evidence. Do not invoke another implementation workflow or expand the Task to perform them.
