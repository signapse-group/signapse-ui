...
- Runtime frontend và `docs/APIMAPPING.md` sở hữu sự thật về tính năng đang khả dụng.
- Tài liệu này sở hữu các requirement có thể kiểm chứng và kế hoạch triển khai của landing.
...
Các requirement về Hero phải khớp baseline `8ae5336`: hai Hero proof points và Hero secondary CTA tới `#how-it-works`; các requirement về bốn product chapters, AnalysisFlow ba bước, trust, media và CTA còn lại giữ theo runtime hiện tại. Không duy trì contract cạnh tranh với tài liệu này.
...
- Không sửa drift ngoài landing trong `docs/APIMAPPING.md` hoặc annotation popup runtime; các debt này thuộc task riêng.
...
| Event-aware Charts       | Biểu đồ theo bối cảnh sự kiện | Event-aware Charts            | Candle chart có event/calendar context khi dữ liệu khả dụng                                                                                                                                       | `app/[lang]/(main)/market-charts/market-chart-workbench.tsx`; `app/lib/market-charts/`; `docs/APIMAPPING.md` mục API market charts |
...
| Connected Market Graph   | Đồ thị quan hệ thị trường     | Connected Market Graph        | Graph của event, asset, news article và narrative                                                                                                                                                 | `app/[lang]/(main)/graph-view/`; `docs/APIMAPPING.md` mục API graph view |
| AI Assistant             | Trợ lý AI                     | AI Assistant                  | Hội thoại text-only có session/history theo active workspace; nhận ngữ cảnh từ Market Knowledge Graph để hỗ trợ đặt câu hỏi thị trường bằng văn bản                                               | `components/market-conversation-assistant/`; `docs/APIMAPPING.md` mục API market query |
...
| Telegram updates         | cập nhật qua Telegram         | Telegram updates              | Cảnh báo tin thị trường, lịch kinh tế và phân tích định kỳ gửi tới điểm nhận đã liên kết và cấu hình                                                                                              | `app/[lang]/(main)/telegram/telegram-configuration.tsx`; `docs/APIMAPPING.md` mục API telegram |
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Workspace và tracked assets        | Người dùng có thể quản lý danh sách tài sản theo dõi của active workspace; chart chọn asset từ danh sách này.                                                                                                             | Chỉ có một tracked-asset list cho active workspace.                                                                                                                                                                                                                                                                         | Named/grouped/nested watchlists; shared team workspace; watchlist là evidence boundary của AI.                                                                                                       | `app/[lang]/(main)/workspaces/`; `app/[lang]/(main)/market-charts/page.tsx`; `docs/APIMAPPING.md` mục workspace/watchlist |
| Event-aware Charts                 | Chart tải historical candles cho asset được chọn từ tracked-asset list và có thể hiển thị market-event annotation cùng economic-calendar event liên quan; live quote hoặc partial candle có thể được cập nhật qua stream. | Annotation và calendar event chỉ xuất hiện khi backend trả dữ liệu phù hợp. Dùng từ “live” riêng cho dữ liệu chart; stream có thể stale, disconnected hoặc market closed, và candle response có thể rỗng.                                                                                                                   | Real-time intelligence toàn hệ thống; annotation/calendar luôn tồn tại; provider uptime guarantee; arbitrary symbol charting; trade entry/stop/target.                                               | `app/[lang]/(main)/market-charts/market-chart-workbench.tsx`; `app/lib/market-charts/`; `docs/APIMAPPING.md` mục API market charts |
...
Quyết định nội dung chốt ngày 2026-09-07: chuyển từ bốn bước sang ba bước bên dưới. Đây là baseline cho thay đổi tiếp theo; runtime và dictionary chưa được cập nhật trong phiên chốt nội dung này.
...
| Telegram          | Text-only, measure tối đa `3xl`            | `app/[lang]/(main)/telegram/telegram-configuration.tsx`; `docs/APIMAPPING.md` mục API telegram |
...
- Cutover-only: mailbox request-access phải được owner xác nhận provision, nhận external mail và có người theo dõi trước apex release; đây không phải completion check của landing implementation task.
...
- Tách thành hai deliverable: task đầu rebuild landing trên application host ở trạng thái public `noindex`; task sau mới thực hiện apex cutover, retire/supersede coming-soon contract và bật indexability.
- Cập nhật requirement hiện có thay vì chồng thêm “V2” requirements.
