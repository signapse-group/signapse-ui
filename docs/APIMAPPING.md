...
| Phuong thuc | Endpoint backend | operationId      | Tich hop frontend         | Trang thai                         | Ghi chu                                                                                                       |
| ----------- | ---------------- | ---------------- | ------------------------- | ---------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| GET         | `/blogs`         | `getBlogPosts`   | `getBlogs(searchParams)`  | Da trien khai                      | Tra ve `Page<BlogPostListResponse>`; producer contract moi dung `status` thay cho `visible`.                  |
| POST        | `/blogs`         | `createBlogPost` | `createBlog(request)`     | Da tich hop theo producer contract | Frontend gui `title`, `slug`, `shortDescription`, `content`, `contentSchemaVersion`; bai moi luon la `DRAFT`. |
...
| DELETE      | `/me/feedback-submissions/{id}`            | `withdraw`     | `withdrawFeedback`                             | Da tich hop | Rut submission cua user; runtime `204`, live OpenAPI publish `200` empty-body gap duoc BE giai thich.                                                                                                                                                  |
...
| POST        | `/feedback-submissions/{id}/review`        | `review`       | `reviewFeedback`                               | Da tich hop | Request chi co `reviewMessage` trim, non-blank, toi da 1.000 ky tu; permission `feedback:review`; transition mot chieu sang `REVIEWED`. Live OpenAPI publish `additionalProperties: false`; minLength van la `0` nen runtime validation giu non-blank. |
...
| DELETE      | `/feedback-submissions/{id}`               | `erase`        | `deleteFeedback`                               | Da tich hop | Xoa o scope quan tri; permission `feedback:delete`; runtime `204`, live OpenAPI publish `200` empty-body gap duoc BE giai thich.                                                                                                                       |

Ghi chu:

- Probe authoritative dev OpenAPI ngày 18/09/2026 (`https://dev-api.signapse.cloud/v3/api-docs`, OpenAPI `3.1.0`, 100 paths, 185 schemas) xác nhận 10 feedback operations trên 7 path, gồm endpoint `review`, status `PENDING_REVIEW`/`REVIEWED`, response detail có `reporter` moderation và không còn `githubIssueNumber`. OpenAPI vẫn thiếu hoặc biểu diễn khái quát filter grammar, request conditionality, success status detail, requiredness/nullability, lifecycle, screenshot constraints và error examples.
...
- `POST /feedback-submissions/{id}/review` chỉ nhận `reviewMessage` trim, non-blank, tối đa `1000`; response chuyển atomically sang `REVIEWED`, message immutable, và conflict dùng `FEEDBACK_ALREADY_REVIEWED`. Live schema đã bỏ `githubIssueUrl` và đặt `additionalProperties: false`, nhưng vẫn publish `minLength: 0`, nên runtime non-blank là phần semantics cần giữ ngoài OpenAPI.
- Status response chỉ có `PENDING_REVIEW` và `REVIEWED`. List/detail dùng `content`; personal detail có `clientContext`/`reviewMessage`, moderation detail thêm `reporter`; `githubIssueNumber` và các field nội bộ đã bị loại bỏ.
...
- OpenAPI chi publish response `200`, khong mo ta error response, state transition guard, ownership failure, gioi han/kieu MIME screenshot, hoac lifecycle cho withdraw/review/erase. Contract runtime ghi nhận `204` cho withdraw/erase, review conflict `409`, ownership/permission boundaries, và lifecycle hai trạng thái; frontend fixture và tests phản ánh contract runtime đó.
...
- `blogs`: live dev OpenAPI van chua deploy producer contract #95, trong khi frontend definitions/actions/forms/list da target `content`, `contentSchemaVersion`, `status`, va khong con visibility field.
