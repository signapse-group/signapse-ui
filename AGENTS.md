...
- Delivery condition and issue-linking rule: Deliver one scoped pull request linked to the exact assigned implementation issue. Handoff only after applicable checks, independent review, and any configured required CI pass for the delivered revision. Do not merge, deploy, close the issue, or move it to `Done`.
